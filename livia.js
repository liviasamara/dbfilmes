...
db.filmes.find({generos:{$in:["Action"]},data_estreia:{$gte:new Date(2001,1,1),$lte:new Date(2003,12,31)}},{_id:0,titulo:1,data_estreia:1,generos:1}).pretty()

// 02 - Filmes de comedia ou terror com menos de 2 horas.

db.filmes.find({$or:[{generos:{$in:["Horror"]}},{generos:{$in:["Comedy"]}}],
                duracao:{$lt:120}},{_id:0,titulo:1,genero:1,duracao:1}).pretty()
...
db.filmes.find({diretores:{$in:["Steven Spier"]},data_estreia: {$gte:new Date(1990,1,1),$lte:new Date(1999,12,31)}},{_id:0,titulo:1,diretores:1,data_estreia:1}).pretty()


// 07 - Qual o filmes com mais de 3 horas que teve a maior arrecadação.

db.filmes.find({duracao:{$gt:180}},{_id:0,titulo:1,duracao:1,arrecadacao:1})
...
db.filmes.count({generos:{$in:["Action"],$in:["Comedy"]},data_estreia:{$gte:new Date(2010,1,1),$lte:new  Date(2010,12,31)}})


// 10 - Qual o filme com Brad Pitt com maior duração.

db.filmes.find({atores:{$in:["Brad Pitt"]},},{_id:0,atores:1,duracao:1}).sort({duracao:-1}).limit(1)
